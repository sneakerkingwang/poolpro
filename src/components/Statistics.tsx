...
const Statistics: React.FC<StatisticsProps> = ({ isAdmin }) => {
  ...
  useEffect(() => {
    console.log("Setting up players listener for statistics...");
    const playersRef = collection(db, "players");
    const q = query(playersRef, orderBy("rank", "asc"));
    
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      console.log("Received players snapshot for statistics");
      const playersData = querySnapshot.docs.map(doc => { 
        ...
        console.log(`Player stats: ${player.name} - 
          Rating: ${player.rating}, Change: ${player.rating - player.previousRating}
          8B: ${player.wins8Ball}/${player.matches8Ball}
          9B: ${player.wins9Ball}/${player.matches9Ball}`);
    ...
    return () => {
      console.log("Cleaning up players listener for statistics");
      unsubscribe();
    };
  }, []);
